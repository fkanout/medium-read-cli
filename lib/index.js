...
const mediumModule = (() => {
  ...
  const getPostById = (postId) => {
    request(`${constants.MEDIUM.URL_GET_ONE_POST}${postsList[postId]}`).then((dirtyResponse) => {
      ...
      if (success) {
        ...
        Object.keys(content).forEach(key => print(content[key].text, content[key].type));
        stdin.resume();
  ...
  const start = () => {
    stdin.on('keypress', (chunk, key) => {
      if (key && key.name === 'backspace') { showArticlList(); } else if (key && key.name === 'space') { getMoreArticles(); } else if (key && key.name === 'escape') {
